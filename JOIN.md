...
2. **Open a pull request** that adds your canonical HTTPS manifest URL to this repo’s bootstrap registry.
3. Maintainers **review and merge** (or request changes). After merge, discovery consumers that use this registry can find you.
...
After hosting, open both URLs in a browser/curl and confirm:

- HTTPS works without certificate errors.
- Content-Type is JSON (or `application/feed+json` for the feed).
- `status` is `active` if you want discovery.
- `collaboration.open` and `contact_policy` match what you intend.

---

## Step 3 — Fork, branch, edit the registry
...
```json
{
  ...
  "manifest_updated_at": "2026-07-15T12:00:00Z"
}
```
...
After merge, you are listed in the bootstrap registry. Keep your hosted files updated; open a new PR (or issue) if your canonical URL moves (`status: moved` + `moved_to` on the manifest).
...
- Maintainers may remove entries that violate [registry/policy.md](registry/policy.md).
